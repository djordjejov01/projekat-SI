...
import { FormArray, FormControl, FormGroup, Validators, ReactiveFormsModule, AbstractControl, ValidationErrors } from '@angular/forms';
...
export class CreateEventComponent implements OnInit, IDeactivate, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.eventForm = new FormGroup({
      ...
      capacity: new FormControl('', [Validators.required, Validators.min(1), Validators.max(2147483647)]),
      ...
      tickets: new FormArray([
        new FormGroup({
          ...
          description: new FormControl('', [CustomValidators.noWhitespaceValidator, Validators.required]),
          quota: new FormControl('', [Validators.required, Validators.min(1), Validators.max(2147483647)]),
    ...
    }, { validators: [CustomValidators.startBeforeEndDates('startDateTime', 'endDateTime'), this.totalQuotaValidator] });
    ...
    this.subscriptions.add(this.eventForm.get('isUnlimitedCapacity')?.valueChanges.subscribe((unlimited) => {
      ...
      this.eventForm.get('tickets')?.updateValueAndValidity();
    }));

    this.subscriptions.add(this.eventForm.get('capacity')?.valueChanges.subscribe(() => {
      this.eventForm.get('tickets')?.updateValueAndValidity();
    }));
  ...
  private totalQuotaValidator(control: AbstractControl): ValidationErrors | null {
    const formGroup = control as FormGroup;
    const isUnlimited = formGroup.get('isUnlimitedCapacity')?.value;
    const capacity = formGroup.get('capacity')?.value;
    const ticketsArray = formGroup.get('tickets') as FormArray;

    if (isUnlimited || !capacity) {
      return null;
    }

    const totalQuota = ticketsArray.controls
      .map(ticketGroup => ticketGroup.get('quota')?.value)
      .reduce((sum, current) => (sum ?? 0) + (current ?? 0), 0);

    return totalQuota > capacity ? { totalQuotaExceedsCapacity: true } : null;
  }
  ...
  submitForm(): void {
    if (this.eventForm.invalid) {
      this.fromValidationService.showValidationErrors(
        this.eventForm,
        this.translateService.instant('Create Event')
      );
      return;
    }
